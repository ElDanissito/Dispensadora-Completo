...
func rawEmail(msgID, machine string, amount string) []byte {
	body := fmt.Sprintf("=C2=A1Listo! Todo sali=C3=B3 bien con tus movimientos Bancolombia: %s, recibiste\r\n"+
		"un pago de Nombre Apellido por $%s en tu cuenta *5322 conectado a la\r\n"+
...
func newPendingOrder(t *testing.T, st *store.Store, jti string, uniqueAmount, at int64) {
	t.Helper()
...
func TestPoll_MatchExpiredButPaidInWindow(t *testing.T) {
	...
	err := st.CreateOrder(ctx, store.Order{
		Jti: "ord_exp", MachineID: "M001", TotalCOP: 2, UniqueAmount: 2,
	...
	svc := New(st, mailer, &fakeEmitter{}, discardLogger())
...
func TestPoll_Orphan_MontoNoCasa(t *testing.T) {
	st, at := setupStore(t)
	ctx := context.Background()
	newPendingOrder(t, st, "ord_x", 2, at) // orden espera 2
