-- Esquema del backend de dispensadoras (contrato-token.md v2) — PostgreSQL.
-- Migrado de SQLite a Postgres (ADR-002 / ADR-020 / ADR-021): despliegue en AWS
-- sobre EC2, con Postgres como contenedor (Docker Compose); RDS al escalar.
-- Base nueva: el CREATE TABLE ya trae TODAS las columnas (no hay ALTER de
-- migración como en el piloto SQLite). Todo con IF NOT EXISTS → Open() idempotente.
-- Los flags booleanos se guardan como INTEGER (0/1) para que el código Go
-- (active == 1, wired == 1) no cambie. Montos y epochs en BIGINT.
...
CREATE TABLE IF NOT EXISTS machine_products (
  ...
  price_cop  BIGINT  NOT NULL,             -- precio en pesos colombianos (entero)
...
--   ambiguous: 2+ ordenes casaron un mismo pago (ADR-018) -> NO se dispensa; revision/soporte.
--   unique_amount: en modo payer = total_cop (monto redondo exacto); en fallback = total_cop + d.
--   payer_name: nombre que el cliente declaro como quien hara la transferencia (ADR-018; PII minima).
CREATE TABLE IF NOT EXISTS orders (
  jti                   TEXT PRIMARY KEY,      -- id único de la orden (= jti del token)
  machine_id            TEXT NOT NULL REFERENCES machines(id),
  total_cop             BIGINT NOT NULL,       -- precio base (suma de ítems)
  unique_amount         BIGINT NOT NULL DEFAULT 0, -- monto exacto a cobrar (ancla del matching)
  payer_name            TEXT NOT NULL DEFAULT '',  -- nombre declarado de quien paga (ADR-018)
  status                TEXT NOT NULL,
  iat                   BIGINT NOT NULL,       -- emitido en (auditoría, solo servidor)
  exp                   BIGINT NOT NULL,       -- expiración del token (se fija al pagar)
  pay_window_expires_at BIGINT NOT NULL DEFAULT 0, -- fin de la ventana de pago (epoch s)
  token                 TEXT,                  -- JWS firmado (se rellena al pagar)
  paid_at               BIGINT,                -- epoch s en que se concilió el pago
...
CREATE TABLE IF NOT EXISTS bank_movements (
  ...
  amount_cop   BIGINT,                     -- monto normalizado a entero COP
  ...
  occurred_at  BIGINT,                     -- fecha/hora del cuerpo (epoch s)
  processed_at BIGINT NOT NULL,            -- cuándo lo procesó la conciliación
