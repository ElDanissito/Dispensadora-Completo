 ...
 * PASO 4: Verificar el token v2 y, si es OK, DISPENSAR moviendo los motores.
 *
 ...
 *   4. NUEVO: si el resultado es OK, recorre los items del token y para cada
 *      uno mueve el motor del slot con LOGICA INVERSA (LOW = mueve) durante un
 *      tiempo acotado por TIMEOUT. Mapa slot->GPIO para el demo:
 *         slot 3 -> D27 (Motor 1)   ·   slot 5 -> D14 (Motor 2)
 *      Asi escanear token-valido (items [{s:3,q:1},{s:5,q:2}]) mueve ambos.
 *
 * ORDEN DEL CONTRATO (§5, regla clave): el jti se marca como usado ANTES de
 * accionar motores. Aqui se cumple porque jti_mark_used() ocurre dentro de
 * verificar_token() (paso 9), que retorna OK ANTES de que dispensemos.
 *
 * TIMEOUT sin sensor: todavia no hay E18-D80NK (eso es el paso 5). Por ahora
 * el "timeout" es el tope de tiempo que el motor esta encendido por unidad
 * (MOTOR_TIMEOUT_MS): un limite de seguridad para que NUNCA quede girando.
 * Cuando llegue el sensor, se cortara antes por confirmacion de caida.
 *
 * RTC: todavia no hay DS3231. NOW fijo = 1752460900 (resultados-esperados.md).
 * SEGURIDAD: la maquina SOLO tiene la llave publica. Anti-reuso de jti en RAM
 * (pendiente NVS). Monocypher: los 4 archivos estan en esta misma carpeta.
 */
...
/* Tiempos de dispensado (demo, sin sensor todavia). */
const unsigned long MOTOR_TIMEOUT_MS = 900;   /* tope de encendido por unidad */
...
/* Mueve el motor de un slot `q` veces (una por unidad). Cada pulso esta
 * acotado por MOTOR_TIMEOUT_MS (tope de seguridad, sin sensor todavia).
 * Logica inversa: LOW mueve, HIGH detiene. Devuelve true si el slot existe. */
static bool dispensar_slot(int slot, int q) {
  int gpio = slot_to_gpio(slot);
  if (gpio < 0) {
    Serial.print("  slot "); Serial.print(slot);
    Serial.println(" SIN MOTOR mapeado -> se omite.");
    return false;
  }
  ...
  for (int u = 0; u < q; u++) {
    ...
    Serial.print(" -> MUEVE por hasta "); Serial.print(MOTOR_TIMEOUT_MS);
    Serial.println(" ms");

    /* Encender (LOW) con tope por tiempo. Cuando exista el E18-D80NK, este
     * bucle saldra antes al confirmar la caida del producto (paso 5). */
    digitalWrite(gpio, MOTOR_MUEVE);
    unsigned long t0 = millis();
    while (millis() - t0 < MOTOR_TIMEOUT_MS) {
      /* aqui, en el paso 5: if (producto_cayo(slot)) break; */
    }
...
static void dispensar_items(const char *items) {
...
void setup() {
  ...
  Serial.println();
  Serial.println("=== PASO 4: verificacion + dispensado en el ESP32 ===");
