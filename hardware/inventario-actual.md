...
3. Escribir **código de prueba incremental** (un paso a la vez, no todo de golpe):
   ...
   5b. [x] **Paso 5b — HECHO (2026-07-16). Anti-reuso de `jti` PERSISTENTE en NVS.** `firmware/paso5b-jti-nvs/` (copia del paso 5 con el registro de `jti` movido de RAM a **NVS**, librería `Preferences`). El `jti` se sigue marcando **antes** de dispensar (§5), pero ahora la escritura se **commitea a flash de forma síncrona**, así que el `ALREADY_USED` **sobrevive a reinicios/apagones**. Orden crash-safe: se guarda el `jti` y **después** el contador `n`; si se corta la energía entre ambos, pasa antes de dispensar → estado consistente, sin doble entrega. Al arrancar recarga la caché desde NVS e imprime `jti usados cargados de NVS: N`. Comando de prueba `!reset` (por USB) borra el registro. **Pendiente ligado al RTC:** poda de `jti` vencidos por `exp` (hoy el registro crece hasta `JTI_MAX=64`; si se llena, avisa por Serial y prioriza no bloquear la venta). Ver [`firmware/paso5b-jti-nvs/README.md`](../firmware/paso5b-jti-nvs/README.md).

   **Prueba de aceptación (paso 5b):** escanear `token-valido`→`OK` → **reiniciar la placa** (arranca con `jti usados cargados de NVS: 1`) → escanear el mismo QR → **`ALREADY_USED`** (antes del 5b daba `OK` de nuevo tras reiniciar).

   **Pendiente de firmware (refinamiento, no bloquea):** **RTC DS3231** para `exp` cuando llegue el módulo (hoy `NOW` fijo). El anti-reuso `jti` en NVS ya está resuelto (paso 5b).
